...
const formatDateFromTimestamp = (timestamp: string | number) => {
  const seconds = typeof timestamp === "number" ? timestamp : parseInt(timestamp);
...
export default function TransactionHistory() {
  ...
  } = useStakesByUser(address ?? undefined);

  const {
    data: unstakes,
    isLoading: loadingUnstakes,
    refetch: refetchUnstakes,
  } = useUnstakesByUser(address ?? undefined);

  const {
    data: rewards,
    isLoading: loadingRewards,
    refetch: refetchRewards,
  } = useRewardClaimsByUser(address ?? undefined);
