...
export default function AnalyticInterface() {
  const { address: userAddress } = useStacksWallet();
  ...
  const { data: rewardClaims, isLoading: loadingRewards } =
    useRewardClaimsByUser(userAddress ?? undefined, 1000);

  const totalRewards = useMemo(() => {
    if (!rewardClaims) return BigInt(0);
    return rewardClaims.reduce(
      (sum, r) => sum + parseBigIntFromGraphQL((r as any).reward ?? (r as any).amount ?? '0'),
      BigInt(0)
    );
  }, [rewardClaims]);
  ...
  const groupedRewards = useMemo(() => {
    ...
    rewardClaims.forEach((r) => {
      ...
      buckets[key] =
        current +
        Number(parseBigIntFromGraphQL((r as any).reward ?? (r as any).amount ?? '0')) /
          1e6;
    });
