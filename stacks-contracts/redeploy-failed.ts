 ...
 * 
 * Redeploys the contracts that failed in the previous deployment
 */
...
// Failed contracts to redeploy
const FAILED_CONTRACTS = [
  'jaga-stake',
  'insurance-manager',
  'dao-governance',
  'claim-manager',
...
async function deployContract(
  ...
  try {
    const txOptions = {
      ...
      fee: 200000, // 0.2 STX - higher fee for faster inclusion
    };
 ...
 */
async function main() {
  console.log('🚀 Jagantara - Redeploying Failed Contracts');
  ...
  FAILED_CONTRACTS.forEach((contract, index) => {
  ...
  for (const contractName of FAILED_CONTRACTS) {
  ...
  if (deployedContracts.length === 0) {
    ...
    console.log(`✅ ${deployedContracts.length}/${FAILED_CONTRACTS.length} contracts deployed`);
    console.log('\n📝 Deployed Contracts:');
